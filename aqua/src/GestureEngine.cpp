...
GestureEngine::GestureEngine(ClientConnection* c, GestureServer* s) {
    _client = c;
    _server = s;
    
    // Ask the client for its global gestures.
...
bool GestureEngine::processEvent(Event* event) {
    ...
    if (event.getType() == EVENT_TYPE_DOWN || event.getType() == OTHER ||
            event.getType() == HOVER) {
        ...
        if (_regionIDToRegionMap.find(regionID) != _gestureMap.end()) {
            ...
            _client.getGestures(regionID, r->getGestures());
        ...
        if (_eventToRegionMap.find(eventID) != _eventToRegionMap.end()) {
            // This region exists.
            _eventToRegionMap[eventID]->processEvent();
         } else {
